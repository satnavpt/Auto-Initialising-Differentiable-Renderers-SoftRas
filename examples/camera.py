...
class SfM_Camera:
	def __init__(self, output_path):
        ...
        self.img_path = osp.join(self.output_path, "images")
        img = imageio.imread(osp.join(img_path, os.listdir(img_path)[0])) #read the first image
		self.h, self.w, _ = img.shape
		self.image_center = np.array([0.5*h, 0.5*w, 1])  #image in homogeneous/projected coordinates (u, v, 1)
	...
	def get_camera_extrinsics(self, is_dense=True):
		"""Reads from reconstruction results, returns the camera heading list for each view point"""
		...
		for image_id, image in reconstruction.images.items():
		    ...
		    R = SfM_Camera.quaternion_to_rotation(image.qvec)
		    T = image.tvec.reshape(3,1)
		    # print("Rotation Matrix: ", R)
		    camera_loc = -np.linalg.inv(R) @ T
		...
		camera_names = natural_sorting(list(camera_loc_dct)) #sort the filenames so that it starts from 0 degree
		# print(camera_names)
		...
		for name, loc in ordered_camera_loc_dct.items():
		    if camera_names.index(name) == 0:
		        ...
		        ordered_camera_angle_dct[name] = vector_ang(loc_vec1, loc_vec2) * 180.0 / np.pi   #radian to degree
	...
	def quaternion_to_rotation(qvec):
	    """
	    Quaternion to full three-dimensional rotation matrix that converts a point in the local reference frame to a point in the global reference frame.
	    """
	    q0 = qvec[0]
	    q1 = qvec[1]
	    q2 = qvec[2]
